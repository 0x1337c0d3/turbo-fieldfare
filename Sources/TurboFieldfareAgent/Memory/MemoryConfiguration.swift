...
public struct ContinuityStorageConfiguration: Sendable, Equatable {
    ...
    ///
    /// `TINYTITAN_MEMORY_CACHE_MIB` sets one for anyone who wants it; at the cap
...
public struct MemoryConfiguration: Sendable, Equatable {
    ...
    public static func fromEnvironment(
        ...
        if let value = environment["TINYTITAN_MEMORY_RETENTION_DAYS"].flatMap(Int.init) {
        ...
        if let value = environment["TINYTITAN_MEMORY_BOOTSTRAP_LIMIT"].flatMap(Int.init) {
            configuration.limits.bootstrapRecords = max(0, value)
        }
        if let value = environment["TINYTITAN_MEMORY_BOOTSTRAP_BYTES"].flatMap(Int.init) {
            configuration.limits.bootstrapBytes = max(0, value)
        }
        if let value = environment["TINYTITAN_MEMORY_TOOL_ROUNDS"].flatMap(Int.init) {
            configuration.maximumToolRounds = max(0, min(value, 16))
        }
        if let value = environment["TINYTITAN_MEMORY_TOOLS"] {
        ...
        if let value = environment["TINYTITAN_MEMORY_JOURNAL_TURNS"].flatMap(Int.init) {
            configuration.journalLimits.turnsPerSession = max(1, value)
        }
        if let value = environment["TINYTITAN_MEMORY_JOURNAL_SESSIONS"].flatMap(Int.init) {
        ...
        if let value = environment["TINYTITAN_MEMORY_LOCAL_FALLBACK"] {
